        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-slate-50">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-slate-500 font-bold animate-pulse">Loading Academy Courses...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="bg-[#f8fafc] min-h-screen flex flex-col font-sans">
            ...
            <section className="bg-[#0066CC] pt-32 pb-20 md:pt-48 md:pb-32 px-4 relative overflow-hidden">
                ...
                <div className="container mx-auto max-w-5xl text-center relative z-10 px-2">
                    ...
                    >
                        Advance Your Career with <br className="hidden md:block" /> Industry-Standard Certification
                    </motion.h1>
                    <motion.p
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        <div className="relative flex items-center bg-white rounded-2xl p-1.5 shadow-[0_20px_50px_rgba(0,0,0,0.15)] focus-within:ring-4 focus-within:ring-blue-400/20 transition-all group">
                            ...
                            <input
                                ...
                                className="flex-1 h-11 md:h-14 bg-transparent border-none focus:ring-0 text-slate-900 font-bold text-[13px] md:text-lg placeholder:text-slate-400"
                                value={searchQuery}
                ...
                <div className="container mx-auto max-w-5xl bg-white rounded-[32px] md:rounded-[40px] shadow-2xl shadow-blue-900/10 border border-slate-100 grid grid-cols-2 lg:grid-cols-4 overflow-hidden">
                    {stats.map((stat, i) => (
                        ...
                        >
                            ...
                            <span className="text-lg md:text-2xl font-black text-slate-900 leading-none mb-1">{stat.value}</span>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-12 gap-4 md:gap-6 border-b border-slate-100 pb-6 md:pb-8">
                        ...
                        <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-slate-100 shadow-sm self-start">
                            <span className="text-blue-600 font-black text-base md:text-lg leading-none">{filteredCourses?.length || 0}</span>
                            <span className="text-slate-400 text-[9px] md:text-[11px] font-black uppercase tracking-wider">Courses Available</span>
                        </div>
                    </div>
                    ...
                    {filteredCourses && filteredCourses.length > 0 ? (
                        ...
                        <div className="text-center py-24 bg-white rounded-[40px] border border-dashed border-slate-200">
                            ...
                            <h3 className="text-2xl font-black text-slate-900 mb-2">No courses found matching "{searchQuery}"</h3>
                            <p className="text-slate-500 font-medium">Try checking your spelling or search for broader keywords like "Java" or "Stack".</p>
                        </div>
                    )}
                </div>
            </section>
