...
export function News({ articles }: { articles: NewsArticle[] }) {
    ...
    // Use [...cards].reverse() instead of toReversed() for better compatibility
    const reversedCards = React.useMemo(() => [...cards].reverse(), [cards]);

    return cards.length || showCompleted ? (
        <div
            className="group overflow-hidden px-3 pb-3 pt-8 h-[500px]"
            data-active={cardCount !== 0}
        >
            <div className="relative size-full">
                {reversedCards.map(({ href, title, summary, image }, idx) => (
                    <div
                        key={href}
                        className={cn(
...
function NewsCard({
    ...
    return (
        <Card
            ref={ref}
            className={cn(
                "relative select-none gap-2 p-3 text-[0.8125rem] h-full flex flex-col",
                "translate-x-[calc(var(--dx)*1px)] rotate-[calc(var(--dx)*0.05deg)] opacity-[calc(1-max(var(--dx),-1*var(--dx))/var(--w)/2)]",
                "transition-shadow data-[dragging=true]:shadow-md bg-white border-slate-200"
            )}
            data-dragging={dragging}
            onPointerDown={onPointerDown}
            onClick={onClick}
        >
            <div className={cn(hideContent && "invisible", "flex flex-col h-full")}>
                <div className="flex flex-col gap-1 mb-2">
                    <span className="line-clamp-1 font-medium text-slate-900 text-lg">
                        {title}
                    </span>
                ...
                <div className="relative flex-grow w-full shrink-0 overflow-hidden rounded-md border border-slate-100 bg-slate-50">
                    {image && (
                        <img
                            src={image}
                            alt=""
                ...
                <div
                    className={cn(
                        "h-0 overflow-hidden opacity-0 transition-[height,opacity] duration-200 mt-2",
                ...
                >
                    <div className="flex items-center justify-between pt-1 text-xs">
                        <span
                            className="font-medium text-blue-600 hover:text-blue-700 transition-colors duration-75"
                        >
                            Swipe to dismiss
                        </span>
                        <button
                            type="button"
                            onClick={dismiss}
                            className="text-slate-400 hover:text-slate-600 transition-colors duration-75"
                        >
                            Dismiss
                        </button>
                    </div>
...
function AnimatedLogo(props: React.SVGProps<SVGSVGElement>) {
    return (
        <svg
            ...
            className="text-slate-300 w-12 h-6"
            {...props}
        >
            <circle cx="24" cy="10.5" r="8" fill="currentColor" />
        </svg>
    );
}
